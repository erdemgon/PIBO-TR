...
const CENTERS = {
  ...
  "MR2":   { label: "Merkez 2", prefix: "MR2", isAdmin: false },
  "MR3":   { label: "Merkez 3", prefix: "MR3", isAdmin: false },
}
...
const FIELD_GROUPS = {
  kimlik: {
    label: "Kimlik & Demografik", fields: [
      {key:"hasta_id", label:"Hasta ID", type:"text", required:true},
      {key:"pibo", label:"PIBO", type:"bool"},
      {key:"ptbo", label:"PTBO", type:"bool"},
      {key:"cinsiyet", label:"Cinsiyet", type:"select", options:[{v:"e",l:"Erkek"},{v:"k",l:"Kız"}]},
      {key:"yabanci", label:"Yabancı uyruklu", type:"bool"},
      {key:"yas_ay", label:"Yaş (ay)", type:"num"},
      {key:"dogum_yil", label:"Doğum yılı", type:"num"},
      {key:"dogum_ay", label:"Doğum ayı", type:"num"},
      {key:"tani_yil", label:"Tanı yılı", type:"num"},
      {key:"tani_ay", label:"Tanı ayı", type:"num"},
    ]
  },
  zaman: {
    label: "Zaman & Tanı Süreci", fields: [
      {key:"tani_yas_gun", label:"Tanı anındaki yaş (gün)", type:"num"},
      {key:"semptom_oncesi_gun", label:"Semptom → tanı (gün)", type:"num"},
      {key:"muayene_tani_gun", label:"Muayene → tanı (gün)", type:"num"},
      {key:"muayene_bronkoskopi_gun", label:"Muayene → bronkoskopi (gün)", type:"num"},
      {key:"semptom_bronkoskopi_gun", label:"Semptom → bronkoskopi (gün)", type:"num"},
    ]
  },
  etiyoloji: {
...
const s = {
  ...
  btnPrimary: { padding:"8px 20px", borderRadius:8, border:"1px solid #3b82f6", background:"#eff6ff", color:"#1d4ed8", cursor:"pointer", fontSize:13, fontWeight:500 },
  ...
  badge: (color) => ({ fontSize:11, padding:"2px 8px", borderRadius:20, background: color==="blue"?"#dbeafe":color==="amber"?"#fef3c7":"#f3f4f6", color: color==="blue"?"#1e40af":color==="amber"?"#92400e":"#374151" }),
}
    ...
    <div style={{minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#f9fafb"}}>
      <div style={{width:380}}>
        <div style={{textAlign:"center", marginBottom:28}}>
          <div style={{fontSize:22, fontWeight:600, color:"#111827"}}>PIBO / PTBO Registry</div>
...
function ActionScreen({ center, centerInfo, patients, onAction, onLogout }) {
  ...
  return (
    <div style={{maxWidth:600, margin:"0 auto", padding:"32px 20px"}}>
      ...
      {centerInfo.isAdmin && (
        <button onClick={() => onAction("admin")} style={{...s.card, width:"100%", textAlign:"left", cursor:"pointer", border:"1px solid #bfdbfe", background:"#eff6ff", marginBottom:14}}>
          <div style={{fontSize:14, fontWeight:500, color:"#1d4ed8"}}>Admin paneli — tüm merkezler, analiz, istatistik →</div>
        </button>
      )}
...
function PatientForm({ patient, isNew, onSave, onBack }) {
  ...
  const [activeGroup, setActiveGroup] = useState("kimlik")
...
// ─── Admin Panel ──────────────────────────────────────────────────────────────
function AdminPanel({ patients, onBack }) {
...
export default function App() {
  ...
  async function savePatient(p) {
    const record = { ...p, merkez: p.hasta_id.split("-")[0], guncelleme_tarihi: new Date().toISOString() }
  ...
  if (screen==="new" || screen==="edit") return (
    <PatientForm
      patient={editing}
      isNew={screen==="new"}
      onSave={async p => { await savePatient(p) }}
      onBack={() => setScreen("action")}
    />
  )

  if (screen==="admin") return (
    <AdminPanel patients={patients} onBack={() => setScreen("action")} />
  )
